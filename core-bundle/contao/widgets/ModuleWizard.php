			...
			foreach ($arrButtons as $button)
			{
				if ($button == 'edit')
				{
					if (($this->varValue[$i]['mod'] ?? null) > 0)
					{
						...
						$return .= ' <a href="' . $href . '" title="' . StringUtil::specialchars($GLOBALS['TL_LANG']['tl_layout']['edit_module']) . '" class="module_link" onclick="Backend.openModalIframe({\'title\':\'' . StringUtil::specialchars(str_replace("'", "\\'", $GLOBALS['TL_LANG']['tl_layout']['edit_module'])) . '\',\'url\':this.href});return false">' . Image::getHtml('edit.svg', '', 'class="module_image"') . '</a>';
					}
					else
					{
						$return .= ' ' . Image::getHtml('edit_.svg', '', 'class="module_image"');
					}
				}
				elseif ($button == 'drag')
				{
